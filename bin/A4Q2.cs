// Dafny program A4Q2.dfy compiled into C#
...
// Dafny 3.2.0.30713
// Command Line Options: c:\Users\fordl\Desktop\SWEN324\Assignments\Assignment 4\A4Q2.dfy /compile:3 /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\A4Q2
// A4Q2.dfy
...
method Main()
{
  var l1: List<nat> := Cons(1, Cons(2, Cons(3, Cons(2, Cons(2, Nil)))));
  var msl1 := list2Multiset(l1);
  print msl1, ""  msl1[2]= "", msl1[2], ""\n"";
  var msl2 := list2Map(l1);
  print msl2, ""\n"";
}

function method list2Multiset<T>(l: List<T>): (r: multiset<T>)
  decreases l
{
  if l.Cons? then
    multiset{l.head} + list2Multiset(l.tail)
  else
    multiset{}
}

function method list2Map<T>(l: List<T>): (r: map<T, nat>)
  decreases l
{
  map x: T {:trigger list2Multiset(l)[x]} | x in list2Multiset(l) :: list2Multiset(l)[x]
}

lemma setEq<T>(l: List<T>, el: T)
  decreases l
{
}

lemma /*{:_induction l}*/ msetEq<T>(l: List<T>, el: T)
  ensures forall x: T :: x in list2Map(l).Keys ==> list2Multiset(l)[x] >= 1
  decreases l
{
}
")]
...
namespace _module {
  ...
  public abstract class List<T> {
    ...
    public static List<T> create_Cons(T head, List<T> tail) {
    ...
    public T dtor_head {
      get {
        var d = this;
        return ((List_Cons<T>)d).head; 
      }
    }
    public List<T> dtor_tail {
      get {
        var d = this;
        return ((List_Cons<T>)d).tail; 
      }
    }
  }
  ...
  public class List_Cons<T> : List<T> {
    ...
    public override bool Equals(object other) {
      ...
      return oth != null && object.Equals(this.head, oth.head) && object.Equals(this.tail, oth.tail);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this.head));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this.tail));
      return (int) hash;
    }
    public override string ToString() {
      string s = "List.Cons";
      s += "(";
      s += Dafny.Helpers.ToString(this.head);
      s += ", ";
      s += Dafny.Helpers.ToString(this.tail);
      s += ")";
      return s;
    }
  }
  ...
  public partial class __default {
    ...
    {
      List<BigInteger> _33_l1;
      _33_l1 = @List<BigInteger>.create_Cons(BigInteger.One, @List<BigInteger>.create_Cons(new BigInteger(2), @List<BigInteger>.create_Cons(new BigInteger(3), @List<BigInteger>.create_Cons(new BigInteger(2), @List<BigInteger>.create_Cons(new BigInteger(2), @List<BigInteger>.create_Nil())))));
      Dafny.IMultiSet<BigInteger> _34_msl1;
      _34_msl1 = __default.list2Multiset<BigInteger>(_33_l1);
      Dafny.Helpers.Print(_34_msl1);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("  msl1[2]= "));
      Dafny.Helpers.Print((_34_msl1).Select(new BigInteger(2)));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Dafny.IMap<BigInteger,BigInteger> _35_msl2;
      _35_msl2 = __default.list2Map<BigInteger>(_33_l1);
      Dafny.Helpers.Print(_35_msl2);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    }
    public static Dafny.IMultiSet<__T> list2Multiset<__T>(List<__T> l) {
      Dafny.IMultiSet<__T> _36___accumulator = Dafny.MultiSet<__T>.FromElements();
    TAIL_CALL_START: ;
      if ((l).is_Cons) {
        _36___accumulator = Dafny.MultiSet<__T>.Union(_36___accumulator, Dafny.MultiSet<__T>.FromElements((l).dtor_head));
        List<__T> _in0 = (l).dtor_tail;
        l = _in0;
        goto TAIL_CALL_START;
      } else {
        return Dafny.MultiSet<__T>.Union(Dafny.MultiSet<__T>.FromElements(), _36___accumulator);
      }
    }
    public static Dafny.IMap<__T,BigInteger> list2Map<__T>(List<__T> l) {
      return Dafny.Helpers.Id<Func<List<__T>, Dafny.IMap<__T,BigInteger>>>((_37_l) => ((System.Func<Dafny.IMap<__T,BigInteger>>)(() => {
        var _coll0 = new System.Collections.Generic.List<Dafny.Pair<__T,BigInteger>>();
        foreach (__T _compr_0 in (__default.list2Multiset<__T>(_37_l)).Elements) {
          __T _38_x = (__T)_compr_0;
          if ((__default.list2Multiset<__T>(_37_l)).Contains((_38_x))) {
            _coll0.Add(new Dafny.Pair<__T,BigInteger>(_38_x,(__default.list2Multiset<__T>(_37_l)).Select(_38_x)));
          }
        }
        return Dafny.Map<__T,BigInteger>.FromCollection(_coll0);
      }))())(l);
    }
  }
