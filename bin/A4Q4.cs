// Dafny program A4Q4.dfy compiled into C#
...
// Dafny 3.2.0.30713
// Command Line Options: c:\Users\fordl\Desktop\SWEN324\Assignments\Assignment 4\A4Q4.dfy /compile:3 /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\A4Q4
// A4Q4.dfy

datatype Tree<T(==)> = Empty | Leaf(v: T) | Node(v: T, l: Tree<T>, r: Tree<T>)

function method space(i: nat): (r: string)
  decreases i
{
  match i
  case 0 =>
...
method toPrnt(t: Tree<int>, s: nat)
  decreases t, s
{
  if t.Empty? {
    print "" . "";
...
method Main()
{
  var t13: Tree<nat> := seqToTree([1, 2, 3]);
  var t14: Tree<nat> := seqToTree([1, 2, 3, 4]);
  var t17: Tree<nat> := seqToTree([1, 2, 3, 4, 5, 6, 7]);
  print t14, ""\n"";
  toPrint(t17);
  print ""[1,2,3] == "", treeToSeq(seqToTree([1, 2, 3])), ""\n"";
  print ""[1,2,3,4] == "", treeToSeq(seqToTree([1, 2, 3, 4])), ""\n"";
  var bad: Tree<nat> := Node(4, Empty, Node(2, Leaf(1), Leaf(3)));
  var badSeq := treeToSeq(bad);
  var back := treeToSeq(seqToTree(badSeq));
  print ""badRobot "", bad, ""   "", badSeq, ""   "", seqToTree(badSeq), ""  "", back, ""\n"";
  var oops := addToTree(bad, 99);
  print ""OPPS "", oops, ""\n"";
  var poor: Tree<nat> := Node(3, Leaf(1), Leaf(5));
  print ""p*** "", addToTree(poor, 98), ""\np*** "", addToTree(addToTree(addToTree(poor, 98), 99), 100), ""\n"";
  print ""[1,2,3,4] == "", treeToSeq(seqToTree([1, 2, 3, 4])), ""\n"";
  var tee: Tree<nat> := seqToTree([1, 2, 3, 4]);
  print tee, ""   "", treeToSeq(tee), ""\n"";
  var t: Tree<nat> := Node(2, Leaf(1), Leaf(3));
  print t, ""\n  ***>  "", treeToSeq(t), ""\n"";
  var ta: Tree<nat> := addToTree(t, 99);
  print ta, ""\n  ***>  "", treeToSeq(ta), ""\n"";
  var t1: Tree<nat> := Node(4, Node(2, Leaf(1), Leaf(3)), Empty);
  print t1, ""\n  ***>  "", treeToSeq(t1), ""\n"";
  var t1a: Tree<nat> := addToTree(t1, 99);
  print t1a, ""\n  ***>  "", treeToSeq(t1a), ""\n"";
  var t2: Tree<nat> := Node(4, Node(2, Leaf(1), Leaf(3)), Node(6, Leaf(5), Empty));
  print t2, ""\n  ***>  "", treeToSeq(t2), ""\n"";
  var t2a: Tree<nat> := addToTree(t2, 99);
  print t2a, ""\n  ***>  "", treeToSeq(t2a), ""\n"";
}

function method treeToSeq<T>(t: Tree<T>): seq<T>
  decreases t
{
  match t {
    case Empty() =>
    ...
    case Leaf(v) =>
      [v]
    case Node(v, l, r) =>
      treeToSeq(l) + [v] + treeToSeq(r)
  }
}

function method seqToTree<T>(s: seq<T>): Tree<T>
  decreases s
{
  Empty
}

function method seqToTre<T>(s: seq<T>, sofar: Tree<T>): Tree<T>
  decreases s, sofar
{
  Empty
}

function method addToTree<T>(t: Tree<T>, a: T): Tree<T>
  decreases t
{
  Empty
}

predicate method treeEqu<T(==)>(t: Tree<T>, u: Tree<T>)
  decreases t, u
{
  true
}
...
namespace _module {
  ...
  public abstract class Tree<T> {
    ...
    public static Tree<T> create_Empty() {
      return new Tree_Empty<T>();
    }
    public static Tree<T> create_Leaf(T v) {
      return new Tree_Leaf<T>(v);
    }
    public static Tree<T> create_Node(T v, Tree<T> l, Tree<T> r) {
      return new Tree_Node<T>(v, l, r);
    }
    public bool is_Empty { get { return this is Tree_Empty<T>; } }
    public bool is_Leaf { get { return this is Tree_Leaf<T>; } }
    public bool is_Node { get { return this is Tree_Node<T>; } }
    public T dtor_v {
      get {
        var d = this;
        if (d is Tree_Leaf<T>) { return ((Tree_Leaf<T>)d).v; }
        return ((Tree_Node<T>)d).v; 
      }
    }
    public Tree<T> dtor_l {
      get {
        var d = this;
        return ((Tree_Node<T>)d).l; 
      }
    }
    public Tree<T> dtor_r {
      get {
        var d = this;
        return ((Tree_Node<T>)d).r; 
      }
    }
  }
  public class Tree_Empty<T> : Tree<T> {
    public Tree_Empty() {
    }
    public override bool Equals(object other) {
      var oth = other as Tree_Empty<T>;
      return oth != null;
    }
    public override int GetHashCode() {
      ulong hash = 5381;
  ...
  public class Tree_Node<T> : Tree<T> {
    public readonly T v;
    public readonly Tree<T> l;
    public readonly Tree<T> r;
    public Tree_Node(T v, Tree<T> l, Tree<T> r) {
      this.v = v;
      this.l = l;
      this.r = r;
    }
    public override bool Equals(object other) {
      ...
      return oth != null && object.Equals(this.v, oth.v) && object.Equals(this.l, oth.l) && object.Equals(this.r, oth.r);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 2;
  ...
  public partial class __default {
    public static Dafny.ISequence<char> space(BigInteger i) {
      Dafny.ISequence<char> _119___accumulator = Dafny.Sequence<char>.FromElements();
    TAIL_CALL_START: ;
      if ((i).Sign == 0) {
        return Dafny.Sequence<char>.Concat(_119___accumulator, Dafny.Sequence<char>.FromString(""));
      } else {
        BigInteger _120_n = i;
        _119___accumulator = Dafny.Sequence<char>.Concat(_119___accumulator, Dafny.Sequence<char>.FromString(" "));
        BigInteger _in0 = (_120_n) - (BigInteger.One);
        i = _in0;
        goto TAIL_CALL_START;
      }
    }
    public static void toPrint(Tree<BigInteger> t)
    {
      __default.toPrnt(t, BigInteger.Zero);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    }
    public static void toPrnt(Tree<BigInteger> t, BigInteger s)
    {
      if ((t).is_Empty) {
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" . "));
      } else if ((t).is_Leaf) {
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" ("));
        Dafny.Helpers.Print((t).dtor_v);
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(") "));
      } else {
        Dafny.Helpers.Print(Dafny.Sequence<char>.Concat(Dafny.Sequence<char>.Concat(Dafny.Sequence<char>.FromString("\n"), __default.space(s)), Dafny.Sequence<char>.FromString("Node (")));
        Dafny.Helpers.Print((t).dtor_v);
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(") "));
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" "));
        __default.toPrnt((t).dtor_l, (s) + (new BigInteger(6)));
        Dafny.Helpers.Print(Dafny.Sequence<char>.Concat(Dafny.Sequence<char>.FromString("\n"), __default.space((s) + (new BigInteger(8)))));
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString(" "));
        __default.toPrnt((t).dtor_r, (s) + (new BigInteger(6)));
        Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("Nend "));
      }
    }
    public static void _Main()
    {
      Tree<BigInteger> _121_t13;
      _121_t13 = __default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3)));
      Tree<BigInteger> _122_t14;
      _122_t14 = __default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3), new BigInteger(4)));
      Tree<BigInteger> _123_t17;
      _123_t17 = __default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3), new BigInteger(4), new BigInteger(5), new BigInteger(6), new BigInteger(7)));
      Dafny.Helpers.Print(_122_t14);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      __default.toPrint(_123_t17);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("[1,2,3] == "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(__default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3)))));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("[1,2,3,4] == "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(__default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3), new BigInteger(4)))));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _124_bad;
      _124_bad = @Tree<BigInteger>.create_Node(new BigInteger(4), @Tree<BigInteger>.create_Empty(), @Tree<BigInteger>.create_Node(new BigInteger(2), @Tree<BigInteger>.create_Leaf(BigInteger.One), @Tree<BigInteger>.create_Leaf(new BigInteger(3))));
      Dafny.ISequence<BigInteger> _125_badSeq;
      _125_badSeq = __default.treeToSeq<BigInteger>(_124_bad);
      Dafny.ISequence<BigInteger> _126_back;
      _126_back = __default.treeToSeq<BigInteger>(__default.seqToTree<BigInteger>(_125_badSeq));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("badRobot "));
      Dafny.Helpers.Print(_124_bad);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   "));
      Dafny.Helpers.Print(_125_badSeq);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   "));
      Dafny.Helpers.Print(__default.seqToTree<BigInteger>(_125_badSeq));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("  "));
      Dafny.Helpers.Print(_126_back);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _127_oops;
      _127_oops = __default.addToTree<BigInteger>(_124_bad, new BigInteger(99));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("OPPS "));
      Dafny.Helpers.Print(_127_oops);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _128_poor;
      _128_poor = @Tree<BigInteger>.create_Node(new BigInteger(3), @Tree<BigInteger>.create_Leaf(BigInteger.One), @Tree<BigInteger>.create_Leaf(new BigInteger(5)));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("p*** "));
      Dafny.Helpers.Print(__default.addToTree<BigInteger>(_128_poor, new BigInteger(98)));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\np*** "));
      Dafny.Helpers.Print(__default.addToTree<BigInteger>(__default.addToTree<BigInteger>(__default.addToTree<BigInteger>(_128_poor, new BigInteger(98)), new BigInteger(99)), new BigInteger(100)));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("[1,2,3,4] == "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(__default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3), new BigInteger(4)))));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _129_tee;
      _129_tee = __default.seqToTree<BigInteger>(Dafny.Sequence<BigInteger>.FromElements(BigInteger.One, new BigInteger(2), new BigInteger(3), new BigInteger(4)));
      Dafny.Helpers.Print(_129_tee);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("   "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_129_tee));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _130_t;
      _130_t = @Tree<BigInteger>.create_Node(new BigInteger(2), @Tree<BigInteger>.create_Leaf(BigInteger.One), @Tree<BigInteger>.create_Leaf(new BigInteger(3)));
      Dafny.Helpers.Print(_130_t);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_130_t));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _131_ta;
      _131_ta = __default.addToTree<BigInteger>(_130_t, new BigInteger(99));
      Dafny.Helpers.Print(_131_ta);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_131_ta));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _132_t1;
      _132_t1 = @Tree<BigInteger>.create_Node(new BigInteger(4), @Tree<BigInteger>.create_Node(new BigInteger(2), @Tree<BigInteger>.create_Leaf(BigInteger.One), @Tree<BigInteger>.create_Leaf(new BigInteger(3))), @Tree<BigInteger>.create_Empty());
      Dafny.Helpers.Print(_132_t1);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_132_t1));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _133_t1a;
      _133_t1a = __default.addToTree<BigInteger>(_132_t1, new BigInteger(99));
      Dafny.Helpers.Print(_133_t1a);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_133_t1a));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _134_t2;
      _134_t2 = @Tree<BigInteger>.create_Node(new BigInteger(4), @Tree<BigInteger>.create_Node(new BigInteger(2), @Tree<BigInteger>.create_Leaf(BigInteger.One), @Tree<BigInteger>.create_Leaf(new BigInteger(3))), @Tree<BigInteger>.create_Node(new BigInteger(6), @Tree<BigInteger>.create_Leaf(new BigInteger(5)), @Tree<BigInteger>.create_Empty()));
      Dafny.Helpers.Print(_134_t2);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_134_t2));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
      Tree<BigInteger> _135_t2a;
      _135_t2a = __default.addToTree<BigInteger>(_134_t2, new BigInteger(99));
      Dafny.Helpers.Print(_135_t2a);
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n  ***>  "));
      Dafny.Helpers.Print(__default.treeToSeq<BigInteger>(_135_t2a));
      Dafny.Helpers.Print(Dafny.Sequence<char>.FromString("\n"));
    }
    public static Dafny.ISequence<__T> treeToSeq<__T>(Tree<__T> t) {
      Tree<__T> _source0 = t;
      if (_source0.is_Empty) {
        return Dafny.Sequence<__T>.FromElements();
      } else if (_source0.is_Leaf) {
        __T _136___mcc_h0 = ((Tree_Leaf<__T>)_source0).v;
        __T _137_v = _136___mcc_h0;
        return Dafny.Sequence<__T>.FromElements(_137_v);
      } else {
        __T _138___mcc_h1 = ((Tree_Node<__T>)_source0).v;
        Tree<__T> _139___mcc_h2 = ((Tree_Node<__T>)_source0).l;
        Tree<__T> _140___mcc_h3 = ((Tree_Node<__T>)_source0).r;
        Tree<__T> _141_r = _140___mcc_h3;
        Tree<__T> _142_l = _139___mcc_h2;
        __T _143_v = _138___mcc_h1;
        return Dafny.Sequence<__T>.Concat(Dafny.Sequence<__T>.Concat(__default.treeToSeq<__T>(_142_l), Dafny.Sequence<__T>.FromElements(_143_v)), __default.treeToSeq<__T>(_141_r));
      }
    }
    public static Tree<__T> seqToTree<__T>(Dafny.ISequence<__T> s) {
      return @Tree<__T>.create_Empty();
    }
    public static Tree<__T> seqToTre<__T>(Dafny.ISequence<__T> s, Tree<__T> sofar)
    {
      return @Tree<__T>.create_Empty();
    }
    public static Tree<__T> addToTree<__T>(Tree<__T> t, __T a)
    {
      return @Tree<__T>.create_Empty();
    }
    public static bool treeEqu<__T>(Tree<__T> t, Tree<__T> u)
    {
      return true;
    }
  }
